...
- **Bootstrap 5.3** — loaded via `bootstrap.bundle.min.js` (includes Popper). Symfony form theme: `bootstrap_5_layout.html.twig`. Use `data-bs-*` attributes, `me-/ms-/pe-/ps-*` spacing, `text-start/end`, `float-start/end`, `fw-bold`, `text-bg-*` badges, `form-select` for selects, `mb-3` instead of `form-group`, `w-100` instead of `btn-block`, `col-12` (not `col-xs-12`), `text-body-secondary` (not `text-muted`), `text-bg-*` (not `bg-* text-white`). Color mode is set globally via `data-bs-theme` on `<html>` (overridable `html_theme` block in `@SpipuUi/base.html.twig`); never set `data-bs-theme` locally on components. Navbars require a `container-fluid` wrapper; the default background is `bg-body-tertiary` (theme-adaptive). For custom navbar backgrounds, pair the `bg-*` class with `spipu-navbar-on-dark` / `spipu-navbar-on-light` (defined in `spipu-ui.css`, named after the background, replacing the deprecated `navbar-dark`). Toggler icon: `fa-solid fa-bars` (not `navbar-toggler-icon`, whose SVG color is hardcoded). Active nav links: class `active` on `<a class="nav-link">` (not on `<li>`). Use native `bootstrap.Modal` API (not jQuery `.modal()`).
- **Bundle CSS** — any new CSS class defined by a bundle must be prefixed with `spipu-`. Use Bootstrap CSS variables (`var(--bs-body-bg)`, `var(--bs-tertiary-bg)`, `var(--bs-border-color)`, ...) instead of hardcoded colors, so styles follow the color mode. Bundle CSS compiled from SCSS (e.g. `DashboardBundle/public/scss/`) must be updated in both files, in sync. Google Charts graphs intentionally keep their native (non theme-aware) colors.
